  ...
  const fetchMembershipData = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.get(
        "/equals-money/countries-currencies"
      );
      if (response?.data?.success === true) {
      ...
      <div className="flex items-center pb-2">
        ...
        <div className="flex-grow">
          <h5 className="text-center font-medium text-[16px] mb-2 text-[#212058] ">
            What country will payee receive?
          </h5>
        </div>
      </div>
        ...
        <div className="bg-white px-[20px] py-[14px]">
          <div className="">
            <div className="mb-[40px]">
              ...
              >
                {/* What country will payee receive in? */}
                Country
              </label>
              <div
              ...
              >
                <select
                  id="country_code"
                  className="max-w-full mx-auto mb-1 cursor-pointer text-center  bg-transparent focus:outline-none text-[16px] font-medium text-[#000000]"
                  {...register("country_code", { required: true })}
                >
                  <option value="">Select</option>
                  {membershipData?.countries &&
                    membershipData?.countries?.length > 0 &&
                    membershipData?.countries?.map((item, index) => {
                      const maxLength = 35; // number of characters before truncating
                      const countryDisplay =
                        item?.country_name.length > maxLength
                          ? item?.country_name.slice(0, maxLength) + "..."
                          : item?.country_name;
                      return (
                        <option
            ...
            <div className="mb-[40px]">
              ...
              >
                ...
                >
                  <option value="">Select</option>
                  {membershipData?.currencies &&
                    membershipData?.currencies?.length > 0 &&
                    membershipData?.currencies?.map((item, index) => {
                      return (
